...
if($selected == 'day') {
	...
	$incomeQuery = mysqli_query($con, " SELECT * FROM `income` WHERE `user_id` = '$userId' AND`date` = '$filterDate' ");
} else if($selected == 'month') {
	$filterMonth = $_COOKIE['filterMonth'].'-01';
	$incomeQuery = mysqli_query($con," SELECT * FROM `income` WHERE `user_id` = '$userId' AND YEAR(`date`) = YEAR('$filterMonth') AND MONTH(`date`) = MONTH('$filterMonth') ");
} else if($selected == 'year') {
	$filterYear = $_COOKIE['filterYear'].'-01'.'-01';
	$incomeQuery = mysqli_query($con," SELECT * FROM `income` WHERE `user_id` = '$userId' AND YEAR(`date`) = YEAR('$filterYear') ");
} else if($selected == 'custom') {
	$filterFromDate = $_COOKIE['filterFromDate'];
	$filterToDate = $_COOKIE['filterToDate'];
	$incomeQuery = mysqli_query($con," SELECT * FROM `income` WHERE `user_id` = '$userId' AND `date` >= '$filterFromDate' AND `date` <= '$filterToDate' ");
}
